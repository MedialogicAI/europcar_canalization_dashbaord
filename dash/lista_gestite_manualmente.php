...
<script src="//cdn.datatables.net/1.10.12/js/jquery.dataTables.min.js"></script>
            ...
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                     <h2>Manually managed Practices (List)</h2>   
                    </div>
                </div>              
                 <!-- /. ROW  -->
                  <hr />
...
select.form-control:not([size]):not([multiple]) {
   ...
   <h1 align="center">Manually managed Practice</h1>
   <br>
	...
	<div class="col-md-4">
		...
		<select id="searchFields" name="searchFields" class="form-control">
			...
			<option value="0">Requested date</option>
			<option value="1">Start Process Date</option>
			<option value="2">Registration Number</option>
			<option value="3">Atar File</option>		
			<option value="4">Process Result</option>	
		</select>
	</div>
	...
	<div class="col-md-4">
		<br>
		<button type="button" class="downloadPratices" style="width: 100%;height: 33px;margin-top: 6px;" onclick="location.href='../fetcher_folder/fetch_download_gestite_manualmente.php';"><i class="fa fa-download" aria-hidden="true"></i>
  Download CSV</button>
	</div>
    ...
    <table id="product_data" class="table table-bordered table-striped">
     <thead>
      <tr>
		...
		<th class="align-top" >Process Result</th>	
       </tr>
     </thead>
    </table>
	...
	if(fieldValue == 0 || fieldValue == 1){
		$("#search").datepicker({
			...
			onSelect: function(dateText) {
				var key = dateText;
				$('#daSvuotare').empty();
				$('#daSvuotare').html('<table id="product_data" class="table table-bordered table-striped"><thead><tr><th style="min-width: 55px" class="align-top" >Requested date</th><th style="min-width: 55px" class="align-top" >Start Process date</th><th style="min-width: 55px" class="align-top" >Registration Number</th><th style="width: 200px;" class="align-top" >Atar File</th><th class="align-top" >Process Result</th></tr></thead></table>');	
...
function load_data(search,field)
 {
  var dataTable = $('#product_data').DataTable({
   ...
   "order":[],
   "ajax":{
    url:"/fetcher_folder/fetch_gestite_manualmente.php",
    type:"POST",
