...
<div id="page-wrapper" >
  ...
  // print_r(capacitaProvincie($chars,$solocitta[0])); die();

    for($i=0;$i<count($solocitta);$i++){

        $impNetwork[] = capacitaProvincie($chars,$solocitta[$i]);
    }

    $impNetwork = implode(",",$impNetwork);
    ...
    <script type="text/javascript" language="javascript" >
        ...
        $(document).ready(function(){
            $( "#comuni" ).change(function(){
                ...
                location.href = '../dash/lista_carico_prov_network.php?com='+PROVINCIA;
            });
        });
    ...
    function elencoRegioni($regione) {
        global $servername;
        global $username;
        global $password;
        global $dbname;
        ...
        $sql="select sum(impegno) as impegno, PROVINCIA FROM `view_completa_network` where REGIONE like '%".$regione."%' GROUP BY PROVINCIA";
    ...
    function capacitaProvincie($regione,$provincia){
        ...
        $sql="SELECT 
                count(*) as capacity
                from canaliz_rete where CODICE_NETWORK is not NULL and CODICE_NETWORK != '' and NETWORK not like '%diretti%' and REGIONE like '%$regione%' and PROVINCIA like '$provincia' GROUP BY `PROVINCIA`";
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                $capacityProv[]= $row['capacity'][0];


            }

            return $capacityProv[0] * 2;
