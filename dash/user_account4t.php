            ...
            <div id="page-inner">
				...
				<div class="row">
					<div class="col-md-12" id="nselection" >
						<div class="col-md-4">
							...
							<select class="form-control comuni" name="selectComuni" id="comuni">
								<option id="label" selected > Select Municipality </option>
								...
								for ($i=0;$i<count($comuni);$i++){ 
									...
									echo '<option id="'.$comuni[$i]['COMUNE'].'">'.$comuni[$i]['COMUNE'].'</option>';
								}
					...
					<div class="col-md-12" id="chartContainer">
							<canvas id="capacityChart" style="height: ; width: 680px;" width="680" height=""></canvas>
					</div>
                 <!-- /. ROW  -->           
            </div>
             <!-- /. PAGE INNER  -->
...
<script type="text/javascript" language="javascript" >
	...
	$(document).ready(function(){	 
		$( "#comuni" ).change(function(){
			var comune = $(this).children(":selected").attr("id");
			location.href = '../dash.php?comune=lista_capacita_comuni.php?com='+comune;
		});			 
	});
...
function elencoComuni ($regione) {	
	...
	$sql="select sum(carico) as capacity, COMUNE , sum(impegno) as impegno FROM `view_completa_diretti` where REGIONE like '%".$regione."%' GROUP BY COMUNE";
