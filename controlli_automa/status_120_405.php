...
$sql = "SELECT id,status,retrybricked,oldstatusbricked FROM `ataraxia_main` where retrybricked = 2 and status = 120";
...
if ($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc()) {
		...
		$sqlUpdate = "UPDATE `ataraxia_main` SET status = 405 ,oldstatusbricked = 120 WHERE id =".$row['id'];
