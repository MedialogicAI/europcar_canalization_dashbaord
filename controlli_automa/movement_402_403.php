...
$sql = "SELECT targa_veicolo, wsm_bsm_dt, status, id FROM `ataraxia_main` WHERE NOW() BETWEEN (wsm_bsm_dt + INTERVAL 1 DAY) AND NOW() AND status = 402";
...
if ($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc()) {
		...
		if ($resultMovement->num_rows > 0){
			...
			$sqlUpdate = "UPDATE ataraxia_main SET status='403' WHERE id='".$row['id']."'";
