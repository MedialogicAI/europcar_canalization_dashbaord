        ...
        if ($loadConfig) {
            ...
            if (count($arrData) != 1) {
                ...
                $this->appConfig = array(
                    "instDir" => 'dashboard2', // base app installation dir, '' if installed on server root dir
	...
	*/
	public function buildSelectFields($arrFields=Array(),$blnApplyFunctions=true) {
		...
		foreach ($arrFields as $field) {
			$fieldDb = explode(".", $field['fieldName']);
			$tabName = '';
			$tabField = $field['fieldName'];
			if (count($fieldDb)>0) {
				$tabName = $fieldDb[0];
				$tabField = $fieldDb[1];
			}
...
//	}
	...
	public function getRegionsUser() {
		...
		if (!$this->isLoggedManager()) {
			if ($this->isLoggedStation()) {
				$this->db->where("COD_GWY_PARTNER", $this->getSessionField("station"));
			}
			else {
	...
	public function getStationsUser($limit=null, $start=null, $search=null) {
		...
		if ($this->isLoggedStation()) {
			$r = new \stdClass();
			$r->station_code = $this->getSessionField("station");
			$arrStations[0] = $r;
			return $arrStations;
		}
		$userField = ($this->isLoggedFleetManager()) ? "REGIONAL_FLEET_MGR" : "FLEET_COORDINATOR";
	...
	public function buildSqlConditionStations($arrStations, $isObj=true, $field="station_code") {
		$blnFirst = true;
		$v = "";
		$condition = "";
		foreach($arrStations as $el) {
			$item = ($isObj) ? $el->$field : $el[$field];
			$condition .= $v."'$item'";
			if ($blnFirst) {
				$blnFirst = false;
				$v = ",";
		...
		return "(ataraxia_main.codice_stazione IN ($condition))";
	}
