...
class ExportController extends MainController {
	...
	private function praticesSearch($searchParams=Array()) {
		...
		$conditionStations = $this->buildSqlConditionStations($arrStationsUser);
		...
		$arrSelectFields = explode(",", $sqlSelectFields);
		$sqlSelectFields = '';
		foreach ($arrSelectFields as &$el) {
			switch(strtolower($el)) {
				case 'activity':
				case 'make_description':
				case 'model_intl_description':
					$el = "flottatab.$el";
					break;
				case 'regione':
					$el = "canaliz_total.$el";
			...
			$sqlSelectFields .= (strlen($sqlSelectFields)>0) ? ", $el" : $el;
		}
