...
class AvailabilityController extends MainController
{
		...
		if ($blnFilterProvinces) {
			...
			if ($blnFilterDirect) {
				...
				$sqlWhereSelect = "WHERE  
													...
													canaliz_rete.EMAIL IS NOT NULL AND movement.REPAIRER_ID IS NOT NULL AND 
													movement.CHECKOUT_STATION = canaliz_canaliz.COD_GWY "; //controllare 
				$sqlWhereNetwork = "(
	...
	public function getData()
	{
		...
		$dynamicServiceCondition = "($carrozzerieCond OR $officineCond OR $gommeCond OR $cristalliCond)";
		...
		if (!$blnFilterRegions && !$blnFilterProvinces) {
			...
			if ($blnFilterNetwork) {
				...
				$sqlOrder = "GROUP BY movement.id ORDER BY canaliz_rete.REGIONE";
				$sqlWhere = "$sqlWhereSelect AND $dynamicServiceCondition AND $conditionsRegions AND $sqlWhereDefect";
				$sql = "$sqlSelect $sqlWhere $sqlOrder";
				$arrMovements = $this->db->query($sql)->result();
				$lastElement = '';
				$arrPresentAdded = array();
		...
		if ($blnFilterRegions) {
			...
			if ($blnFilterNetwork) {
				...
				$sqlWhereSelect = "WHERE
								...
								movement.CHECKOUT_STATION = canaliz_canaliz.COD_GWY ";
				$sqlOrder = "GROUP BY movement.id ORDER BY canaliz_rete.PROVINCIA";
				$sqlWhere = "$sqlWhereSelect AND $dynamicServiceCondition AND $conditionsProvinces AND $sqlWhereDefect";
				$sql = "$sqlSelect $sqlWhere $sqlOrder";
	...
	//___________________prova 
	// private function getNetworkAvailability($x) {
	// $this->db->select("campoCapacita");
	// $this->db->from("tbX");
	// $this->db->where("identificativo",$x);
	// $arrData = $this->db->get()->result();
	// return (count($arrData)>0) ? intval($arrData[0]->carico) : $this->defaultNetworkAvailability;
	// }
