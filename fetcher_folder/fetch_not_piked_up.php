...
$query .= " AND STATUS in (401) AND dt_insert BETWEEN ('2019-04-01') AND NOW()";
...
{
 $query = "SELECT DATE_FORMAT(t1.ts_in_dt,'%d/%m/%Y %T') as tsindtloc, DATE_FORMAT(t1.start_processing,'%d/%m/%Y %T') as start_processing_loc, t1.targa_veicolo, t1.atar_file, t2.long_description, t1.observations FROM ataraxia_main as t1 LEFT JOIN tab_status as t2 ON t1.status = t2.actual_status WHERE STATUS in (401) AND dt_insert BETWEEN ('2019-04-01') AND NOW() ORDER BY `ts_in_dt` DESC";
