...
$cartellafileexport = "download_temp/lista_odierne".$fscdata.".csv";
...
// Parte funzioni PHP
	...
	function pendingselection () {	
		...
		$sql = "SELECT
					...
					t1.status
				FROM ataraxia_main as t1
				LEFT JOIN tab_status as t2 ON t1.status = t2.actual_status
				WHERE dt_insert BETWEEN DATE_FORMAT(NOW(),'%Y-%m-%d') AND NOW()
				order by ts_in_dt desc";
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				if($row["status"]>0 && $row['status']<400){
					$ogniriga[$indekfsc][] = 'Processing pratices';
				}
				if($row["status"] > 400 && $row["status"] != 410 && $row["status"] != 440 && $row["status"] != 460 ){
					$ogniriga[$indekfsc][] = 'Manually addressed';
				}
				else{
