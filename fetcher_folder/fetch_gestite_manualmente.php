...
//$query = "select DATE_FORMAT(ts_in_dt,'%d/%m/%Y %T') as tsindtloc,  DATE_FORMAT(dt_insert,'%d/%m/%Y %T') as dt_insert, targa_veicolo, atar_file, status  FROM `ataraxia_main` ";
$query = "SELECT DATE_FORMAT(t1.ts_in_dt,'%d/%m/%Y %T') as tsindtloc, DATE_FORMAT(t1.start_processing,'%d/%m/%Y %T') as start_processing_loc, t1.targa_veicolo, t1.atar_file, t2.long_description, t1.observations FROM ataraxia_main as t1 LEFT JOIN tab_status as t2 ON t1.status = t2.actual_status";
...
$order = array("ts_in_dt", "start_processing","targa_veicolo","atar_file","observations"); 
$field1 = "";
if(isset($_REQUEST["search"]) && isset($_REQUEST["field"])) 
{
	
	$field = trim($order[$_REQUEST["field"]]);
		if($_REQUEST["field"] == 0 or $_REQUEST["field"] == 1){
...
$query .= " AND STATUS in (490,540,570,450,600,405,560) AND dt_insert BETWEEN ('2019-04-01') AND NOW()";
...
{
 $query = "SELECT DATE_FORMAT(t1.ts_in_dt,'%d/%m/%Y %T') as tsindtloc, DATE_FORMAT(t1.dt_insert,'%d/%m/%Y %T') as dt_insert, t1.targa_veicolo, t1.atar_file, t2.long_description FROM ataraxia_main as t1 LEFT JOIN tab_status as t2 ON t1.status = t2.actual_status where STATUS in (490,540,570,450,600,405,560) AND dt_insert BETWEEN ('2019-04-01') AND NOW()";
