...
$query = "SELECT t1.codice_stazione, DATE_FORMAT(t1.ts_in_dt,'%d/%m/%Y %T') as ts_in_dt_loc, DATE_FORMAT(t1.start_processing,'%d/%m/%Y %T') as start_processing_loc, t1.targa_veicolo, t1.atar_file, t1.defect_code ,t1.email_inviata_autofficina, t2.RAGIONE_SOCIALE FROM ataraxia_main as t1 LEFT JOIN canaliz_rete as t2 ON t1.email_inviata_autofficina = t2.EMAIL";
...
$query .= " WHERE 1=1 ";
$order = array("codice_stazione", "ts_in_dt", "start_processing","targa_veicolo","atar_file","defect_code","RAGIONE_SOCIALE","email_inviata_autofficina"); 
$field1 = "";
...
$query .= " AND t1.status in (400,401,402,403) AND dt_insert BETWEEN ('2019-04-01') AND NOW() ";
...
while($row = mysqli_fetch_array($result))
{
 ...
 $sub_array[] = mb_convert_encoding($row["defect_code"], 'UTF-8', 'UTF-8');
	 if($row["RAGIONE_SOCIALE"] == ''){
		$sub_array[] = "<font color='red' >".mb_convert_encoding($row["defect_code"], 'UTF-8', 'UTF-8')."</font>";
	 }
	 else{
		$sub_array[] = mb_convert_encoding($row["RAGIONE_SOCIALE"], 'UTF-8', 'UTF-8');
	 }
...
function get_all_data($connect)
{
 ...
 $query = "SELECT t1.codice_stazione, DATE_FORMAT(t1.ts_in_dt,'%d/%m/%Y %T') as ts_in_dt_loc, DATE_FORMAT(t1.dt_insert,'%d/%m/%Y %T') as dt_insert, t1.targa_veicolo, t1.atar_file, t1.email_inviata_autofficina, t2.RAGIONE_SOCIALE FROM ataraxia_main as t1 LEFT JOIN canaliz_rete as t2 ON t1.email_inviata_autofficina = t2.EMAIL WHERE t1.status = '400' AND dt_insert BETWEEN ('2019-04-01') AND NOW()";
