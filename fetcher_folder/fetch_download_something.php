...
$cartellafileexport = "download_temp/lista_rejected".$fscdata.".csv";
...
// Parte funzioni PHP
	...
	function pendingselection () {	
		...
		$sql = "SELECT t1.codice_stazione, DATE_FORMAT(t1.ts_in_dt,'%d/%m/%Y %T') as ts_in_dt_loc, DATE_FORMAT(t1.start_processing,'%d/%m/%Y %T') as start_processing_loc, t1.targa_veicolo, t1.atar_file, t2.long_description, t1.observations FROM ataraxia_main as t1 LEFT JOIN tab_status as t2 ON t1.status = t2.actual_status where  STATUS in (410,440,460) AND dt_insert BETWEEN ('2019-04-01') AND NOW() ORDER BY `ts_in_dt` DESC";
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				$ogniriga[$indekfsc][] = (string)$row["observations"];
