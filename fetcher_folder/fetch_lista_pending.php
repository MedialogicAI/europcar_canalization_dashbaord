...
$query="SELECT codice_stazione, DATE_FORMAT(t1.ts_in_dt,'%d/%m/%Y %T') as tsindtloc, DATE_FORMAT(t1.start_processing,'%d/%m/%Y %T') as start_processing_loc, t1.targa_veicolo, t1.atar_file,t2.long_description FROM ataraxia_main as t1 LEFT JOIN tab_status as t2 ON t1.status = t2.actual_status";
...
$query .= " AND `status` = 400 AND wsm_bsm_status like '%Reminder Sent%' ";
...
while($row = mysqli_fetch_array($result))
{
 ...
 $sub_array[] = 'Registration number still in IRM after 12hrs';
...
function get_all_data($connect)
{
 ...
 $query="select codice_stazione,DATE_FORMAT(ts_in_dt,'%d/%m/%Y %T') as ts_in_dt, DATE_FORMAT(dt_insert,'%d/%m/%Y %T') as dt_insert_loc, targa_veicolo, atar_file, status  FROM `ataraxia_main` WHERE dt_insert >= NOW() - INTERVAL 1 DAY AND `status` = 400 AND wsm_bsm_status like 'First Reminder Sent%' order by dt_insert desc ";
